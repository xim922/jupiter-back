...
app.get("/job/all/:company_id/:page/:limit", (req, res) => {
  ...
  db.query(
    `SELECT  * FROM job WHERE company_id=${id} and deleted=0 order by job_id DESC limit ${start}, ${limit} `,
    (err, result) => {
      if (result?.length > 0) {
...
app.post("/persons", (req, res) => {
  ...
  db.query(`SELECT * FROM persons WHERE dni=?`, [dni], (err, result) => {
    if (err) {
      res.send({
        status: 500,
        message: err,
...
app.get("/applications/:jobId", (req, res) => {
  ...
  db.query(
    `SELECT J.title,P.*,JPA.salary FROM persons P 
